...
class OpenAIAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        while iteration < max_iterations:
            ...
            try:
                ...
                if message.tool_calls:
                    ...
                    await task_updater.update_status(
                        ...
                        message=task_updater.new_agent_message(
                            [TextPart(text='Processing FinOps analysis...')]
                        ),
                    )
                ...
                error_parts = [
                    TextPart(
                        text=f'Sorry, an error occurred while processing the FinOps request: {e!s}'
                    )
                ]
        ...
        if iteration >= max_iterations:
            error_parts = [
                TextPart(
                    text='Sorry, the FinOps analysis has exceeded the maximum number of iterations.'
                )
            ]
            await task_updater.add_artifact(error_parts)
            await task_updater.complete()
    ...
    async def execute(
        ...
        logger.debug('[FinOps Agent] execute exiting')
